...
public class FirstPersonAIO : NetworkBehaviour {
    ...
    public override void OnStartLocalPlayer()
    {
        ...
        LocalPlayerModel model = GetComponentInChildren<LocalPlayerModel>();
    ...
    private void Start(){
        ...
        if(!GetComponent<AudioSource>() && !GetComponentInChildren<AudioSource>()) { gameObject.AddComponent<AudioSource>(); }
                ...
                if(IsGrounded)
                {
                    ...
                    } else
                    {
                        if(headbobCycle > nextStepTime)
                        {
                            nextStepTime = headbobCycle + 0.5f;
                            int n = Random.Range(0, footStepSounds.Count);
                            if (footStepSounds.Any() && footStepSounds[n])
                            {
                                audioSource.PlayOneShot(footStepSounds[n],Volume/10);
                                NetworkClient.Send(new FootstepNetworkMessage(audioSource.transform.position));
                            }
    ...
    public void ServerPlayFootstep(Vector3 messagePosition)
    {
        var list = footStepSounds;
        if (list == null || list.Count == 0) return;
        audioSource.PlayOneShot(list[(int)Random.Range(0, list.Count)], Volume);
    }
}
    ...
    public class FPAIO_Editor : Editor{
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            t.audioSource = (AudioSource)EditorGUILayout.ObjectField("Audio Source:", t.audioSource, typeof(AudioSource), true);
            EditorGUILayout.Space();
