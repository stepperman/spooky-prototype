...
namespace QTea
{
	...
	public class AnimatorWalkerController : Controller {
		...
		[SerializeField] private Animator animatorRig;
		...
		private Vector3 animatorTransformOffset;
		...
		void Awake () {
			...
			animatorTransformOffset = transform.position - animatorRig.transform.position;
		...
		{
			animatorRig.transform.position = tr.position - animatorTransformOffset;
			animatorRig.transform.rotation = tr.rotation;
		}
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			{
				_velocity += tr.right * horizontalInput;
				_velocity += tr.forward * verticalInput;
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * horizontalInput;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * verticalInput;
			}*/
		...
		protected virtual Vector3 CalculateMovementVelocity()
		{
			...
			animatorRig.SetFloat("X", _velocity.x);
			animatorRig.SetFloat("Y", _velocity.y);
			bool walking = _velocity.sqrMagnitude > 0.1 * 0.1;
			animatorRig.SetBool("Walking", walking);

			if (!walking) _velocity = Vector3.zero;

			return animatorRig.velocity;
		}
		...
		void OnGroundContactLost()
		{
			...
			Vector3 _currentVelocity = GetMomentum() + Vector3.ClampMagnitude(savedMovementVelocity, Mathf.Clamp(0/*movementSpeed*/ - _horizontalMomentumSpeed, 0f, 0/*movementSpeed*/));
			...
			if(_length >= 0/*movementSpeed*/ * airControl)
				_length -= 0/*movementSpeed*/ * airControl;
			else
				_length = 0f;
